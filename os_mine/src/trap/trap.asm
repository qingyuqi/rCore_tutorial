...
.macro SAVE_ALL
    ...
    # 此时， sp sscratch 都保存内核栈

    # 否则，说明 sp!=0，说明从用户态进入中断， 要切换栈
...
trap_from_user:
    # 提前分配栈帧
    addi sp, sp, -36*XLENB
    # 按照地址递增的顺序，保存除x0、x2以外的通用寄存器
    # x0 恒为零，x2 为sp寄存器，需要特殊处理
    STORE x1, 1
    STORE x3, 3
    STORE x4, 4
    STORE x5, 5
...
.macro RESTORE_ALL
    ...
    # s2 = sepc
    LOAD s2, 33
    # 如果从内核态进入中断，sstatus 的 SPP 位会被硬件设为1
    # 如果从用户态进入中断，sstatus 的 SPP 位会被硬件设为0
    # 取出 sstatus 的 SPP
    andi s0, s1, 1 << 8
    # 若 SPP=0，则代表从用户态进入中断，进行 _to_user 进行额外处理
    bnez s0, _to_kernel
_to_user:
    # 释放在内核栈上分配的内存
    addi s0, sp, 36*XLENB
